...
class TestLocalRuntime:
    ...
    def test_exec_with_cwd(self, tmp_path):
        ...
        rt = LocalRuntime()
        result = rt.exec(
            f'"{sys.executable}" -c "import pathlib; print(\'\\n\'.join(p.name for p in pathlib.Path(\'.\').iterdir()))"',
            cwd=str(tmp_path),
        )
        assert result.success
...
class TestCreateRuntime:
    ...
    def test_network_flag_propagated(self, tmp_path):
        rt = create_runtime(sandbox=True, repo_path=str(tmp_path), network=True)
        assert isinstance(rt, DockerRuntime)
...
class TestDockerRuntimeUnit:
    ...
    def test_network_none_disabled_when_network_enabled(self, tmp_path):

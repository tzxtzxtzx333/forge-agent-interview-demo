...
class _Cp936Stdout(io.TextIOWrapper):
    def __init__(self):
        super().__init__(io.BytesIO(), encoding="cp936", errors="strict")

    def isatty(self):
        return False


class TestWindowsCliEncoding:
    def test_run_welcome_is_cp936_safe(self, monkeypatch, tmp_path):
        from entry import cli as cli_module

        class StopAfterWelcome(Exception):
            pass

        def fake_build_components(*args, **kwargs):
            raise StopAfterWelcome()

        monkeypatch.setattr(cli_module, "_build_app_components", fake_build_components)
        monkeypatch.setattr("sys.stdout", _Cp936Stdout())

        with pytest.raises(StopAfterWelcome):
            cli_module.cli.main(
                args=["run", "--repo", str(tmp_path), "--task", "demo", "--provider", "ollama", "--model", "llama3"],
                prog_name="agent",
                standalone_mode=False,
            )

    def test_chat_welcome_is_cp936_safe(self, monkeypatch, tmp_path):
        from entry import cli as cli_module
        ...
        cli_module.cli.main(
            args=["chat", "--repo", str(tmp_path), "--provider", "ollama", "--model", "llama3"],
            prog_name="agent",
            standalone_mode=False,
        )

...
class TestEventLogWriteAndReplay:
    ...
    def test_task_id_with_underscore_preserved_in_all_events(self, tmp_log_dir):
        task = Task(
            task_id="task_with_underscore",
            description="test",
            repo_path="/tmp/repo",
        )
        action = Action(
            action_type=ActionType.TOOL_CALL,
            thought="run shell",
            tool_call=ToolCall(name="shell", params={"cmd": "echo ok"}),
        )
        log = EventLog.create(task, log_dir=str(tmp_log_dir))
        ...
        events = log.replay()
        assert {event.task_id for event in events} == {"task_with_underscore"}
...
class TestEventLogContextManager:
    ...
    def test_open_existing_preserves_original_task_id(self, sample_task, tmp_log_dir):
        with EventLog.create(sample_task, log_dir=str(tmp_log_dir)) as log:
            log.log_task_start(sample_task)
            path = log.path

        with EventLog.open_existing(path) as reopened:
            reopened.log_task_failed(steps=1, reason="retry")

        raw_events = [json.loads(line) for line in path.read_text(encoding="utf-8").splitlines()]
        assert {event["task_id"] for event in raw_events} == {sample_task.task_id}

...
class TestTokenBudget:
    ...
    def test_trim_history_keeps_tool_call_and_tool_result_together(self):
        budget = TokenBudget()
        msgs = [
            {"role": "user", "content": "task"},
        ...
        token_limit = (
            estimate_tokens(json.dumps(msgs[0], ensure_ascii=False))
            + estimate_tokens(json.dumps(msgs[2], ensure_ascii=False))
            + estimate_tokens(json.dumps(msgs[3], ensure_ascii=False))
            + 5
        )
        result = budget.trim_history(msgs, token_limit=token_limit)

        assert any(m.get("tool_call") for m in result)
        assert any(m.get("tool_call_id") == "call_1" for m in result)

    def test_trim_history_never_keeps_orphan_tool_message(self):
        budget = TokenBudget()
        msgs = [
            {"role": "user", "content": "task"},
        ...
        orphan_tools = [
            m for m in result
            if m.get("role") == "tool" and not any(
                prev.get("tool_call", {}).get("call_id") == m.get("tool_call_id")
                for prev in result
            )
        ]
        assert orphan_tools == []

...
class TestIntegration:
    ...
    def test_full_run_with_all_features(self, tmp_path):
        ...
        registry = (
            ...
            .register(FileReadTool(repo_root=tmp_path))
            .register(FileWriteTool(repo_root=tmp_path))
        )
...
class TestStructuredToolHistory:
    def test_tool_history_is_stored_as_structured_messages(self, tmp_path):
        task = Task(
            task_id="structured_history",
            description="fix",
            repo_path=str(tmp_path),
            max_steps=5,
        )
        ...
        assistant_tool_call = next(m for m in second_call_messages if getattr(m, "tool_call", None))
        tool_result = next(m for m in second_call_messages if m.role == "tool")

        assert assistant_tool_call.tool_call.name == "shell"
        assert assistant_tool_call.tool_call.call_id is not None
        assert tool_result.tool_call_id == assistant_tool_call.tool_call.call_id


class TestConfigSemantics:
    def test_task_max_steps_wins_over_agent_config(self, tmp_path):
        task = Task(
            task_id="cfgsteps",
            description="fix",
            repo_path=str(tmp_path),
        ...
        registry = ToolRegistry().register(NoopTool("shell"))
        script = [
            Action(ActionType.TOOL_CALL, "step1", ToolCall("shell", {"cmd": "echo 1"})),
            Action(ActionType.FINISH, "done", message="ok"),
        ]
        backend = MockBackend(script)

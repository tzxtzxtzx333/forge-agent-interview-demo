...
class TestFileReadTool:
    def _tool(self, repo_root: Path) -> FileReadTool:
        return FileReadTool(repo_root=repo_root)

    def test_read_existing_file(self, sample_file):
        result = self._tool(sample_file.parent).execute({"path": str(sample_file)})
        assert result.success
        assert "line 0" in result.output
        assert "1 |" in result.output     # 行号格式

    def test_read_nonexistent_file(self, tmp_path):
        result = self._tool(tmp_path).execute({"path": str(tmp_path / "ghost.py")})
        assert not result.success
        assert "not found" in result.error.lower()

    def test_read_directory_fails(self, tmp_path):
        result = self._tool(tmp_path).execute({"path": str(tmp_path)})
        assert not result.success

    def test_large_file_truncated(self, large_file):
        result = self._tool(large_file.parent).execute({"path": str(large_file)})
        assert result.success
    ...
    def test_tool_schema_valid(self):
        schema = self._tool(Path.cwd()).to_llm_schema()
...
class TestFileViewTool:
    def _tool(self, repo_root: Path) -> FileViewTool:
        return FileViewTool(repo_root=repo_root)

    def test_view_from_start(self, large_file):
        result = self._tool(large_file.parent).execute({"path": str(large_file), "start_line": 1})
        assert result.success
    ...
    def test_view_middle_section(self, large_file):
        result = self._tool(large_file.parent).execute({"path": str(large_file), "start_line": 200})
        assert result.success
    ...
    def test_view_last_window_shows_end(self, large_file):
        result = self._tool(large_file.parent).execute({"path": str(large_file), "start_line": 550})
        assert "End of file" in result.output

    def test_view_start_line_beyond_eof(self, sample_file):
        result = self._tool(sample_file.parent).execute({"path": str(sample_file), "start_line": 9999})
        assert not result.success
        assert "exceeds" in result.error.lower()

    def test_view_nonexistent_file(self, tmp_path):
        result = self._tool(tmp_path).execute({"path": str(tmp_path / "missing.py")})
        assert not result.success
...
class TestFileWriteTool:
    def _tool(self, repo_root: Path) -> FileWriteTool:
        return FileWriteTool(repo_root=repo_root)

    def test_write_new_file(self, tmp_path):
        path = tmp_path / "new.py"
        result = self._tool(tmp_path).execute({"path": str(path), "content": "x = 1\ny = 2\n"})
        assert result.success
    ...
    def test_write_overwrites_existing(self, tmp_path):
        ...
        self._tool(tmp_path).execute({"path": str(path), "content": "new content"})
        assert path.read_text() == "new content"

    def test_write_creates_parent_dirs(self, tmp_path):
        path = tmp_path / "deep" / "nested" / "file.py"
        result = self._tool(tmp_path).execute({"path": str(path), "content": "hello"})
        assert result.success
        assert path.exists()

    def test_write_returns_line_count(self, tmp_path):
        path = tmp_path / "f.py"
        result = self._tool(tmp_path).execute({"path": str(path), "content": "a\nb\nc\n"})
        assert "3 lines" in result.output

    def test_write_empty_content(self, tmp_path):
        path = tmp_path / "empty.py"
        result = self._tool(tmp_path).execute({"path": str(path), "content": ""})
        assert result.success
